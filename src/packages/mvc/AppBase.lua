...
function AppBase:enterScene(sceneName, param, transition, time, more)
    local view = self:createView(sceneName, param)
    view:showWithScene(transition, time, more)
    return view
end

function AppBase:createView(name, param)
    for _, root in ipairs(self.configs_.viewsRoot) do
        ...
        if status and (t == "table" or t == "userdata") then
            return view:create(self, name, param)
        end
    end
